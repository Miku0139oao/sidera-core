...
func (b *webBridge) ServeHTTP(writer http.ResponseWriter, request *http.Request) {
	...
	switch {
	case b.admin != nil && (request.URL.Path == adminRoutePrefix || strings.HasPrefix(request.URL.Path, adminRoutePrefix+"/") || strings.HasPrefix(request.URL.Path, subscriptionRoutePrefix)):
